...
parser = argparse.ArgumentParser(
    description='Patch-free 3D Medical Image Segmentation.')
parser.add_argument('-dataset_path',
                    type=str,
                    default='/newdata/why/BraTS20',
                    help='path to dataset')
parser.add_argument('-model_save_to',
                    type=str,
                    default='.',
                    help='path to output')
...
parser.add_argument('-w_sr',
                    type=float,
                    default=0.5,
                    help='w_sr of the lossfunc')
parser.add_argument('-w_tf',
                    type=float,
                    default=0.5,
                    help='w_tf of the lossfunc')
...
if pt.cuda.device_count() > 1:
    if batch_size < pt.cuda.device_count():
        ...
        print('Batch size has to be larger than GPU#. Set to {:d} instead.'.
              format(batch_size))
...
train_dataset = pt.utils.data.DataLoader(trainset,
                                         ...
                                         shuffle=True,
                                         drop_last=True)
val_dataset = pt.utils.data.DataLoader(valset,
                                       batch_size=1,
                                       shuffle=True,
                                       drop_last=True)
...
scheduler = pt.optim.lr_scheduler.ReduceLROnPlateau(optimizer,
                                                    mode='max',
                                                    patience=20)
...
def ValModel():
    ...
    for i, data in enumerate(val_dataset):
        ...
        guidance = pt.autograd.Variable(guidance).type(
            pt.FloatTensor).cuda().unsqueeze(1)
        mask = pt.autograd.Variable(mask).type(
            pt.FloatTensor).cuda().unsqueeze(1)
        for a in range(0, img_size[0] - crop_size[0] + 1,
                       crop_size[0] // 2):  # overlap0.5
            for b in range(0, img_size[1] - crop_size[1] + 1,
                ...
                for c in range(0, img_size[2] - crop_size[2] + 1,
                    ...
                    inputs3D = pt.autograd.Variable(
                        inputs[:, a:(a + crop_size[0]), b:(b + crop_size[1]),
                               c:(c + crop_size[2])]).type(
                                   pt.FloatTensor).cuda().unsqueeze(1)
                    with pt.no_grad():
...
for x in range(epoch):
    ...
    for data in iterator:
        ...
        labels_seg = pt.autograd.Variable(labels_seg).type(
            pt.FloatTensor).cuda().unsqueeze(1)
        labels_sr = pt.autograd.Variable(labels_sr).type(
            pt.FloatTensor).cuda().unsqueeze(1)
        ...
        loss = lossfunc_dice(outputs_seg, labels_seg) + loss_seg + w_sr * (
            loss_sr + loss_guide) + w_tf * loss_pf
        ...
        if args.v:
            ...
            final_img[:,
                      0:(2 *
                         size)] = outputs_seg.cpu().data.numpy()[0, 0, size //
                                                                 2, :, :] * 255
            final_img[:, (2 * size):(
                4 * size)] = outputs_sr.cpu().data.numpy()[0, 0, size //
                                                           2, :, :] * 255
            final_img[:, (4 * size):(
                6 * size)] = labels_seg.cpu().data.numpy()[0, 0, size //
                                                           2, :, :] * 255
            final_img[:, (6 * size):(
                8 * size)] = labels_sr.cpu().data.numpy()[0, 0, size //
                                                          2, :, :] * 255
            final_img[:, (8 * size):] = cv2.resize(
                inputs.cpu().data.numpy()[0, 0, size // 4, :, :],
                ((2 * size), (2 * size))) * 255
    ...
    if dice > best_dice:
        ...
        print(
            ...
            '/PFSeg_3D_BraTS_patch-free_bs' + str(batch_size) + '_best.pt')
        pt.save(
            model.state_dict(), model_path + '/PFSeg_3D_BraTS_patch-free_bs' +
