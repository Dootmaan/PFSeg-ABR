...
class GuidedMVILiverDataset3D(torch.utils.data.Dataset):
    def __init__(self, path, mode='train', augment=True):
        ...
        self.mode = mode
        self.aug = augment
        images = sorted(glob.glob(path + "/*_PV.nii"))
        labels = sorted(glob.glob(path + "/*_PV_liver.nrrd"))
        ...
        n_test = min(
            len(images) - n_train - n_val, int(test_frac * len(images)))
        ...
        if mode == 'train':
            ...
            for i in range(len(images)):
                ...
                image = self.reset_spacing(image,
                ...
                lesion_arr[lesion_arr != 1] = 0  # only liver
    ...
    def cropMR(self, img, mask):
        ...
        return self.normalization(
            ndimage.interpolation.zoom(
                img, [
                    img_size[0] / img.shape[0], img_size[1] / img.shape[1],
                    img_size[2] / img.shape[2]
                ],
                order=1)), self.normalization(
                    ndimage.interpolation.zoom(
                        mask, [
                            2 * img_size[0] / img.shape[0], 2 * img_size[1] /
                            img.shape[1], 2 * img_size[2] / img.shape[2]
                        ],
                        order=0)), self.normalization(
                            ndimage.interpolation.zoom(img, [
                                2 * img_size[0] / img.shape[0],
                                2 * img_size[1] / img.shape[1],
                                2 * img_size[2] / img.shape[2]
                            ],
                                                       order=1))
    ...
    def crop_guidance(self, label_sr):
        ...
        mask = np.zeros(label_sr.shape).astype(np.float64)
        mask[D - guide_img_size[0] // 2:D + guide_img_size[0] // 2,
             M - guide_img_size[1] // 2:M + guide_img_size[1] // 2,
             N - guide_img_size[2] // 2:N + guide_img_size[2] // 2] = 1
        return label_sr[D - guide_img_size[0] // 2:D + guide_img_size[0] // 2,
                        M - guide_img_size[1] // 2:M + guide_img_size[1] // 2,
                        N - guide_img_size[2] // 2:N +
                        guide_img_size[2] // 2], mask
    ...
    def augment(self, img, label_seg, label_sr):
        ...
        if random.random() < 0.5:  #Shift
            ...
            for i in range(img.shape[0]):
                img[i, :, :] = cv2.warpAffine(img[i, :, :], M_img,
                                              (img.shape[1], img.shape[2]))
            for i in range(label_seg.shape[0]):
                label_seg[i, :, :] = cv2.warpAffine(
                    label_seg[i, :, :], M_label,
                    (label_seg.shape[1], label_seg.shape[2]))
                label_sr[i, :, :] = cv2.warpAffine(
                    label_sr[i, :, :], M_label,
                    (label_sr.shape[1], label_sr.shape[2]))

        if random.random() < 0.5:  #Rotate
            degree = np.random.randint(0, 360)
            M_img = cv2.getRotationMatrix2D(
                ((img.shape[1] - 1) / 2.0, (img.shape[2] - 1) / 2.0), degree,
                1)
            M_label = cv2.getRotationMatrix2D(((label_seg.shape[1] - 1) / 2.0,
                                               (label_seg.shape[2] - 1) / 2.0),
                                              degree, 1)
            for i in range(img.shape[0]):
                img[i, :, :] = cv2.warpAffine(img[i, :, :], M_img,
                                              (img.shape[1], img.shape[2]))
        ...
        return img[start_z:(start_z + crop_size[0]),
                   start_x:(start_x + crop_size[1]),
                   start_y:(start_y + crop_size[2])], label_seg[
                       2 * start_z:2 * (start_z + crop_size[0]),
                       2 * start_x:2 * (start_x + crop_size[1]),
                       2 * start_y:2 * (start_y + crop_size[2])], label_sr[
                           2 * start_z:2 * (start_z + crop_size[0]),
                           2 * start_x:2 * (start_x + crop_size[1]),
                           2 * start_y:2 * (start_y + crop_size[2])]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train' or self.mode == 'all':
            if self.aug:
                data, label_seg, label_sr = self.augment(
                    self.data[index], self.label_seg[index],
                    self.label_sr[index])
                guidance, mask = self.crop_guidance(self.label_sr[index])
                return data, label_seg, label_sr, guidance, mask
            else:
                guidance, mask = self.crop_guidance(self.label_sr[index])
                return self.data[index], self.label_seg[index], self.label_sr[
                    index], guidance, mask
        else:
            guidance, mask = self.crop_guidance(self.label_sr[index])
            return self.data[index], self.label_seg[index], self.label_sr[
...
if __name__ == '__main__':
    dataset = GuidedMVILiverDataset3D('/newdata/why/MVI_Liver_Formatted',
                                      train=True)
